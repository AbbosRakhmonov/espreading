...
const colorPalettes = [
  ["#85C1E9", "#F9E79F", "#F5B7B1", "#A9DFBF", "#D2B4DE", "#2ECC71"],
  ["#76D7C4", "#F8C471", "#AED6F1", "#FADBD8", "#F7DC6F", "#2ECC71"],
  ["#A3E4D7", "#BB8FCE", "#FDEBD0", "#F1948A", "#F9E79F", "#2ECC71"],
  ["#F8C471", "#AED6F1", "#F5CBA7", "#D7BDE2", "#A9DFBF", "#2ECC71"],
];
...
const Dashboard = () => {
    ...
    return [
      {
        ...
        percentage: ((distribution[0] / total) * 100).toFixed(1),
      },
      {
        label: "Score 1",
        value: distribution[1],
        percentage: ((distribution[1] / total) * 100).toFixed(1),
      },
      {
        label: "Score 2",
        value: distribution[2],
        percentage: ((distribution[2] / total) * 100).toFixed(1),
      },
      {
        label: "Score 3",
        value: distribution[3],
        percentage: ((distribution[3] / total) * 100).toFixed(1),
      },
      {
        label: "Score 4",
        value: distribution[4],
        percentage: ((distribution[4] / total) * 100).toFixed(1),
      },
      {
        label: "Score 5",
        value: distribution[5],
        percentage: ((distribution[5] / total) * 100).toFixed(1),
      },
  ...
  const totalReadings = new Set(readingsByLesson.map((r) => r._id.reading))
    .size;
      ...
      <Container maxWidth="xl" sx={{ py: 2, pt: 4 }}>
          ...
          <CardContent>
            ...
            <Grid container spacing={3}>
              {universityStats.map((uni, index) => {
                const scoreData = getScoreDistributionByUniversity(
                  uni.university,
                );
                const avgScore = avgScoreByUniversity.find(
                  (a) => a._id === uni.university,
                );
                      ...
                      <Box sx={{ mb: 2 }}>
                        ...
                        {avgScore && (
                          <Typography
                            ...
                            align="center"
                          >
                            Avg Score: {avgScore.avgScore.toFixed(2)} |
                            Readings: {avgScore.totalReadings}
                          </Typography>
                        )}
                      </Box>
                      {scoreData.length > 0 ? (
                ...
                <TableBody>
                  {readingsByLesson.map((reading) => {
                    ...
                    const completionPercentage =
                      total > 0
                        ? ((reading.completed / total) * 100).toFixed(1)
                        : 0;

                    return (
                      <TableRow
                        key={`${reading._id.lesson}-${reading._id.category}-${reading._id.reading}`}
                      >
                        ...
                        <TableCell align="right">
                          {reading.avgScore
                            ? reading.avgScore.toFixed(2)
                            : "N/A"}
                        </TableCell>
                        <TableCell align="right">
                          {reading.avgTime
                            ? formatTime(Math.round(reading.avgTime))
                            : "N/A"}
                        </TableCell>
                        <TableCell>
                          <Box
                ...
                <Table>
                  ...
                  <TableBody>
                    {recentCompletions.map((completion, index) => (
                      <TableRow key={index}>
                        <TableCell>
                          {completion.user?.fullName || "N/A"}
                        </TableCell>
                        <TableCell>
                          {completion.user?.university || "N/A"}
                        </TableCell>
                        <TableCell>
        ...
        {questionnaireStats && (
          <>
            ...
            <Card sx={{ mb: 4 }}>
              <CardContent>
                ...
                <Grid container spacing={3}>
                      ...
                      <Paper sx={{ p: 2 }}>
                        ...
                        <Box sx={{ mt: 2, textAlign: "center" }}>
                          <Typography variant="body2">
                            ...
                            {questionnaireStats.pre.overall.globAverage.toFixed(
                              2,
                            )}{" "}
                            ({questionnaireStats.pre.overall.globLevel}) |{" "}
                            <strong>Problem Solving:</strong>{" "}
                            {questionnaireStats.pre.overall.probAverage.toFixed(
                              2,
                            )}{" "}
                            ({questionnaireStats.pre.overall.probLevel}) |{" "}
                            <strong>Support:</strong>{" "}
                            {questionnaireStats.pre.overall.supAverage.toFixed(
                              2,
                            )}{" "}
                            ({questionnaireStats.pre.overall.supLevel})
                          </Typography>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                          ...
                          >
                            3.5 or higher = High | 2.5 – 3.4 = Medium | 2.4 or
                            lower = Low
                          </Typography>
                        </Box>
                      </Paper>
                  ...
                  {/* Post-Questionnaire */}
                  {questionnaireStats.post &&
                    questionnaireStats.post.overall && (
                      <Grid size={{ xs: 12, md: 6 }}>
                        <Paper sx={{ p: 2 }}>
                          <Typography variant="h6" align="center" gutterBottom>
                          ...
                          <Box sx={{ mt: 2, textAlign: "center" }}>
                            <Typography variant="body2">
                              <strong>Global:</strong>{" "}
                              {questionnaireStats.post.overall.globAverage.toFixed(
                                2,
                              )}{" "}
                              ({questionnaireStats.post.overall.globLevel}) |{" "}
                              <strong>Problem Solving:</strong>{" "}
                              {questionnaireStats.post.overall.probAverage.toFixed(
                                2,
                              )}{" "}
                              ({questionnaireStats.post.overall.probLevel}) |{" "}
                              <strong>Support:</strong>{" "}
                              {questionnaireStats.post.overall.supAverage.toFixed(
                                2,
                              )}{" "}
                              ({questionnaireStats.post.overall.supLevel})
                            </Typography>
                            <Typography
                              variant="caption"
                              color="text.secondary"
            ...
            {questionnaireStats.pre &&
              questionnaireStats.pre.byInstitution &&
              questionnaireStats.pre.byInstitution.length > 0 && (
                <Card sx={{ mb: 4 }}>
                  <CardContent>
                    <Typography
                      variant="h5"
                    ...
                    <Grid container spacing={3}>
                      {questionnaireStats.pre.byInstitution.map(
                        (inst, index) => (
                          <Grid
                          ...
                          >
                            <Paper sx={{ p: 2, height: "100%" }}>
                              ...
                              >
                                {inst.count}{" "}
                                {inst.count === 1 ? "response" : "responses"}
                              </Typography>
                              <PieChart
                                colors={
                              ...
                              <Box sx={{ mt: 2, textAlign: "center" }}>
                                <Typography variant="body2">
                                  <strong>Global:</strong>{" "}
                                  {inst.globAverage.toFixed(2)} (
                                  {inst.globLevel}) |{" "}
                                  <strong>Problem Solving:</strong>{" "}
                                  {inst.probAverage.toFixed(2)} (
                                  {inst.probLevel}) | <strong>Support:</strong>{" "}
                                  {inst.supAverage.toFixed(2)} ({inst.supLevel})
                                </Typography>
                                <Typography
                                  variant="caption"
                                  color="text.secondary"
                                  sx={{ mt: 1, display: "block" }}
                                >
                                  3.5 or higher = High | 2.5 – 3.4 = Medium |
                                  2.4 or lower = Low
                                </Typography>
                              </Box>
                            </Paper>
            ...
            {questionnaireStats.post &&
              questionnaireStats.post.byInstitution &&
              questionnaireStats.post.byInstitution.length > 0 && (
                <Card sx={{ mb: 4 }}>
                  <CardContent>
                    <Typography
                      variant="h5"
                    ...
                    <Grid container spacing={3}>
                      {questionnaireStats.post.byInstitution.map(
                        (inst, index) => (
                          <Grid
                            size={{ xs: 12, md: 6, lg: 4 }}
                            key={inst.university}
